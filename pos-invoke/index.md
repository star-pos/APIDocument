...
| 订单号 | String | order\_no | 非必填  第三方传入的订单号<br>订单号的格式必须是由英文和数字组成的字符串（不要使用特殊字符）| 与传过来的一致 | 用法一：消费、扫码支付时作为订单号传入做交易   <br>用法二：扫码撤销时与批次流水号任选其一传入做撤销（两者都不传入或无之前使用该订单号的本地交易数据，则调起收单扫码撤销页面） |
